...
#Funciones auxiliares (para mayor modularidad)
def serialize(obj):
...
def sp_falta_general(ini_date, fin_date):
    conn = db.engine.raw_connection()
    try:
        cursor = conn.cursor()
        cursor.callproc('ObtenerFaltasGenerales', [ini_date, fin_date])
        marcas = cursor.fetchall()
        col_names = [desc[0] for desc in cursor.description]
        results = [dict(zip(col_names, row)) for row in marcas]
    finally:
...
def sp_falta_user(ini_date, fin_date,iduser):
    conn = db.engine.raw_connection()
    try:
        cursor = conn.cursor()
        cursor.callproc('ObtenerFaltasUsuario', [ini_date, fin_date,iduser])
        marcas = cursor.fetchall()
        col_names = [desc[0] for desc in cursor.description]
        results = [dict(zip(col_names, row)) for row in marcas]
    finally:
...
def get_falta_user():
    ...
    results = sp_falta_user(ini_date,fin_date,data['idUser'])
    if not results:
        print("No se encontraron marcaciones para el rango de fechas especificado.")
...
@reportes_bp.route('/falta_general', methods=['POST'])
def get_falta_general():
    data = request.json
    ini_date, fin_date, error = validar_formatear_fechas(data)
    if error:
        return jsonify({'error': error}), 400

    print(f"Filtrando faltas generales desde {ini_date} hasta {fin_date}")

    results = sp_falta_general(ini_date,fin_date)
    if not results:
        print("No se encontraron marcaciones para el rango de fechas especificado.")
    ...
    titulo = f"Reporte de faltas generales del {ini_date.date()} hasta {fin_date.date()}"
    cabeceras = ['Nombre completo', 'Fechas faltadas', 'Nro. Días faltados']

    buffer = crear_pdf(titulo,cabeceras,2.4,serializable_results)
    return send_file(buffer, as_attachment=True, download_name="reporte_falta_general.pdf", mimetype='application/pdf')