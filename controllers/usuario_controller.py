...
@usuario_bp.route('/all_users', methods=['GET'])
def all_users():
...
def facial_login():
    ...
    # Convertir el encoding de la cara del usuario desde la base de datos
    user_face_encoding = np.frombuffer(user.face_encoding, dtype=np.float64)
    
    # Comparar la cara detectada con el encoding del usuario
